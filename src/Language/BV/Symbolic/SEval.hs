...
sevalExpr = go where
  ...
  go env (Fold (BVFold { bvfLambda = (larg0, larg1, le0), .. })) =
      ...
      in foldl' (\b a -> go ((larg0, a):(larg1, b):env) le0) i bytes
  go env (Op1 op1 e0) =
      let !v0 = go env e0 in
      case op1 of
          Not   -> snot v0
...
sevalExprStd :: BVExpr -> Sword
sevalExprStd = sevalExpr [('x', inputx), ('y', inputy), ('z', inputz)] where
  inputx = V.fromList [B i | i <- [1..64]]
  inputy = V.fromList [B (65 + i) | i <- [0..63]]
  inputz = V.fromList [B (130 + i) | i <- [0..63]]
{-# INLINABLE sevalExprStd #-}

like :: BVExpr -> BVExpr -> Bool
like !e0 !e1 = e0 == e1 || (slike `on` sevalExprStd) e0 e1
{-# INLINE like #-}
