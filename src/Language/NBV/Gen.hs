...
module Language.NBV.Gen where

import Language.NBV.Types
import Language.NBV.Translator
import qualified Language.BV.Types as BVT
import Language.BV.Util (exprSize)

nexprSize :: NBVExpr -> Int
nexprSize = exprSize . translate

genExpr :: [String] -> Int -> [NBVExpr]
genExpr ops =
    let specgen = (m Map.!)
        BVT.BVOpTags { .. } = BVT.opTagsFromList ops
