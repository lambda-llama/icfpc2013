...
def dump_problems_list():
    d = get_problem()
    d = sorted(filter(lambda p: "solved" not in p, get_problem()), key=(lambda i: 0 if "solved" in i and i["solved"] == True else i["size"] + 0.1 * len(i["operators"])))
...
def generate_inputs_10(out, size=None):
    ...
    for i, (id_s, size, oper) in enumerate(results):
        if sum("sh" in o for o in oper) > 2 and len(oper) < 8:
            fs = 8
        elif len(oper) > 8:
            fs = 8
        else:
